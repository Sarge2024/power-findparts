...
export default function Catalog({
  ...
  return (
    <div id="catalog-section" className="space-y-6">
      ...
      <nav id="breadcrumbs" className="bg-surface-container-low px-4 py-3 rounded-md flex flex-wrap items-center gap-2 overflow-x-auto no-scrollbar border border-outline-variant/10">
        ...
        <div className="flex items-center gap-1.5 whitespace-nowrap">
          ...
          <select 
            ...
            className="text-xs font-headline font-extrabold text-primary bg-transparent border-none p-0 focus:ring-0 cursor-pointer hover:text-surface-tint uppercase"
          >
          ...
          <ChevronRight className="w-3.5 h-3.5 text-outline" />
          <span className="text-xs font-headline font-bold text-primary">{currentVehicle.year}</span>
          
          <ChevronRight className="w-3.5 h-3.5 text-outline" />
          <span className="text-xs font-headline font-bold text-tertiary-container bg-tertiary-fixed px-2 py-0.5 rounded text-[11px] font-black uppercase">
            {selectedGroup}
          </span>
        </div>
      </nav>
      ...
      <section id="catalog-hero" className="relative grid grid-cols-1 md:grid-cols-12 gap-6 items-center bg-white p-6 rounded-md border border-outline-variant/15 overflow-hidden shadow-sm">
        <div className="md:col-span-7 z-10 space-y-3">
          <p className="text-[10px] uppercase tracking-widest font-extrabold text-surface-tint">Base Técnica Sagacitas</p>
          <h2 className="font-display text-3xl md:text-4xl font-extrabold tracking-tight text-primary leading-tight">
            Precisão de Peças Vinculadas.
          </h2>
          <p className="text-xs md:text-sm text-on-surface-variant leading-relaxed max-w-xl">
            Navegue em grupos técnicos automotivos de alta densidade para o Volkswagen <span className="font-bold text-primary">{currentVehicle.model} ({currentVehicle.year})</span>. Filtre por categorias abaixo para selecionar componentes específicos do catálogo.
          </p>
        </div>
        <div className="hidden md:block md:col-span-5 relative h-36 overflow-hidden bg-surface-container rounded-sm border-b-2 border-primary">
      ...
      <section id="bento-categories" className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {techGroups.map((group) => {
          ...
          return (
            <div
              ...
              className={`group relative p-5 rounded-md transition-all cursor-pointer border hover:-translate-y-0.5 active:scale-95 duration-200 ${
                isActive 
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <span className={`material-symbols-outlined text-2xl ${isActive ? 'text-tertiary-fixed font-semibold' : 'text-outline'}`}>
                  {group.icon}
                </span>
              ...
              <h3 className={`font-headline font-bold text-base ${isActive ? 'text-white' : 'text-primary'}`}>
                {group.name}
              </h3>
              <p className={`text-[10px] mt-1 font-semibold uppercase tracking-tighter ${isActive ? 'text-tertiary-fixed' : 'text-on-surface-variant/80'}`}>
                {group.subgroupCount} Subgrupos Vinculados
              </p>
              {isActive && (
      ...
      <section id="parts-inventory" className="space-y-4">
        ...
        {/* Precision Table */}
        <div className="overflow-hidden bg-white border border-outline-variant/15 rounded-md shadow-sm">
      ...
      <section id="diagnostics-summary" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Tonal OBD-II Diagnostic Card */}
        <div className="lg:col-span-2 bg-[#f2f4f5] p-6 rounded-md relative overflow-hidden border border-outline-variant/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <div className="flex items-end gap-2">
                  <span className="text-4xl font-headline font-black text-primary tracking-tight">{obdVoltage}V</span>
                  <span className="text-xs text-outline mb-1 font-medium">— 14.4V Nominal</span>
                </div>
                ...
                <p className={`text-[10px] font-bold uppercase ${obdVoltage < 13.0 || obdVoltage > 14.4 ? 'text-error' : 'text-primary/60'}`}>
                  Status: {voltageAlert}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-black text-primary uppercase tracking-tight">API CONECTADA - SINCRONIZAÇÃO ATIVA</span>
                </div>
                <p className="text-[11px] text-on-surface-variant/80 italic leading-relaxed">
                  As atualizações do catálogo para a Região VWB (BR) estão sendo recuperadas e validadas em tempo real do repositório mestre centralizado da Sagacitas Industrial.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-[#eceeef] p-6 rounded-md flex flex-col justify-between border border-outline-variant/10">
          <div>
            <h5 className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant/60 mb-5">VÍNCULOS DE VEÍCULOS RECENTES</h5>
            <ul className="space-y-3.5">
              {recentLogs.slice(0, 3).map((log) => (
                <li 
                  key={log.id} 
                  onClick={() => onNavigateToTab('inventory')}
                  className="flex items-center justify-between group cursor-pointer hover:bg-white/40 p-2.5 rounded transition-all"
                >
                  ...
                  <ArrowRight className="w-4 h-4 text-outline group-hover:text-primary transition-colors transform group-hover:translate-x-0.5" />
                </li>
              ))}
            </ul>
          </div>
          <button 
            onClick={() => onNavigateToTab('inventory')}
            className="mt-6 border-2 border-primary text-primary px-4 py-2.5 text-[10px] font-black uppercase tracking-widest hover:bg-primary hover:text-on-primary transition-all active:scale-95 rounded-sm"
          >
